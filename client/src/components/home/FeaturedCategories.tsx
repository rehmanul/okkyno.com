    ...
    return (
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <h2 className="font-bold text-2xl md:text-3xl text-center mb-10">What Would You Like To Grow?</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
          {plantCategories.map((category) => (
            <Link key={category.id} href={`/category/${category.slug}`} className="group">
              <div className="aspect-square rounded-lg overflow-hidden relative mb-3 bg-white shadow-md">
                <img 
                  src={category.imageUrl || '/images/categories/vegetables_default.svg'} 
                  alt={category.name} 
              ...
              <h3 className="font-semibold text-center group-hover:text-primary transition duration-300">{category.name}</h3>
            </Link>
          ))}
        </div>
      </div>
