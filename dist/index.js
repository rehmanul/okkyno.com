...
var MemStorage = class {
  ...
  async createSubscriber(insertSubscriber) {
    ...
    const subscriber = {
      ...insertSubscriber,
      id,
      dateSubscribed: /* @__PURE__ */ new Date()
    };
  ...
  async search(query) {
    ...
    const matchedProducts = Array.from(this.products.values()).filter(
      (product) => product.name.toLowerCase().includes(lowercaseQuery) || product.description && product.description.toLowerCase().includes(lowercaseQuery)
    );
    const matchedArticles = Array.from(this.articles.values()).filter(
      (article) => article.title.toLowerCase().includes(lowercaseQuery) || article.content.toLowerCase().includes(lowercaseQuery) || article.excerpt && article.excerpt.toLowerCase().includes(lowercaseQuery)
    );
    const matchedCategories = Array.from(this.categories.values()).filter(
      (category) => category.name.toLowerCase().includes(lowercaseQuery) || category.description && category.description.toLowerCase().includes(lowercaseQuery)
    );
    return {
  ...
  initializeSampleData() {
    const categories2 = [
      ...
      // Subcategories for Tools
      {
        ...
        parentId: 6
        // Assuming Tools is category ID 6
      },
      {
        name: "Pruning Tools",
        slug: "pruning-tools",
    ...
    const products2 = [
      ...
      },
      {
        ...
        slug: "heat-mat",
        description: "Waterproof heat mat that warms soil 10-20\xB0F above ambient temperature to improve germination rates and seedling growth.",
    ...
    const articles2 = [
      {
        ...
        <p>Direct sow seeds in the garden after all danger of frost has passed and soil temperatures reach at least 60\xB0F. Plant seeds 1 inch deep and 2-3 inches apart in rows 18-24 inches apart. For continuous harvests, succession plant every 2-3 weeks.</p>
        ...
        imageUrl: "/images/categories/3139i_Bean-Bush-Goldrush-ORG-new2025_ndzu9h.svg",
        datePublished: /* @__PURE__ */ new Date("2023-05-12"),
        categoryId: 1
      },
      {
        ...
        imageUrl: "/images/categories/2026i_Nasturtium-Fiesta-Blend_6ceu81.svg",
        datePublished: /* @__PURE__ */ new Date("2023-05-08"),
        categoryId: 2
      },
      {
        ...
        imageUrl: "/images/categories/10331_Medium_Tall_LightClay_Compressed.svg",
        datePublished: /* @__PURE__ */ new Date("2023-05-03"),
        categoryId: 5
      }
    ];
    const newArticles = [
      {
        ...
        datePublished: /* @__PURE__ */ new Date("2023-03-10"),
        categoryId: 1
      },
      {
        ...
        datePublished: /* @__PURE__ */ new Date("2023-03-15"),
        categoryId: 6
      },
      {
        ...
        datePublished: /* @__PURE__ */ new Date("2023-04-22"),
        categoryId: 7
      },
      {
        ...
        datePublished: /* @__PURE__ */ new Date("2023-06-05"),
        categoryId: 1
      },
      {
        ...
        datePublished: /* @__PURE__ */ new Date("2023-02-18"),
        categoryId: 6
      }
    ];
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/categories/:categoryId/products", async (req, res) => {
  ...
  app2.get("/api/categories/:categoryId/articles", async (req, res) => {
    const categoryId = parseInt(req.params.categoryId);
    if (isNaN(categoryId)) {
      return res.status(400).json({ message: "Invalid category ID" });
    }
    const articles2 = await storage.getArticlesByCategory(categoryId);
    res.json(articles2);
  });
  ...
  app2.get("/download/epic-gardening-project", (req, res) => {
    const filePath = path.resolve("./public/epic_gardening_project.zip");
